/**
 * AGP CODENAMES -- "كود نيمز" (لعبة أصلية داخل المنصة، بنمط games/khazna
 * و games/team-war من ناحية طريقة التحميل، بدون js/agp-game-shell.js).
 * الشكل البصري منسوخ من تصاميم Claude Design المعتمدة؛ منطق المباراة
...
(function (AGP) {
    ...
    // "طريقة اللعب" -- نافذة مشتركة (زر بالوبي الأساسي + زر داخل إعدادات
    // المباراة). عدّل HOW_TO_PLAY_HTML بس، بدون لمس بقية هذا القسم.
    var HOW_TO_PLAY_HTML = '<p style="text-align:center;color:rgba(240,228,255,0.5);font-size:14px">' +
        '🎮تقدر تسوي ريست للكلمات بس قبل تبدا تاكد من كل السباي ماستر كلماتهم مرتبه يفضل لكل سباي مره وحدة ريست دليل لعبة "كود نيمز" للشات🚨 تنبيه: لا تضغط على شاشة اللعبة حتى يكتمل الإعداد!1. إعداد الأدوار:من أسماء اللاعبين، اضغط علامة (السباي ماستر) لتحديد الملمّح، وعلامة (الصح) لتحديد مؤكد الإجابات للفريقين.2. ربط السباي ماستر:افتح الباركود من الإعدادات ⬅️ يمسحه السباي ماستر للفريقين بالهاتف ⬅️ يختارون ألوانهم ويرسلون طلباً ⬅️ من الإعدادات اقبل الطلبين فقط لتظهر لهم الكلمات الملونة في هواتفهم.3. أوامر الشات:السباي ماستر: يكتب التلميح (الرقم ثم الكلمة) ⬅️ مثال: 3 مدن.اللاعبون: يكتبون (الرقم فقط) لتحديد الصندوق، وللتراجع: سحب [الرقم].المؤكد: يفتح الصناديق بكتابة (تأكيد + الرقم) ⬅️ مثال: تأكيد 12 أو عدة أرقام: تأكيد 15 7 8.' +
    '</p>';
    ...
    function connectionBadgeHtml() {
        ...
        if (saved) {
            return '<div class="cn-conn-badge cn-conn-badge--yes"><span class="cn-conn-dot"></span>' +
                'متصل بالبث <b>@' + escapeHtml(saved.username) + '</b></div>';
        }
        return '<div class="cn-conn-badge cn-conn-badge--no"><span class="cn-conn-dot"></span>' +
            'لم يتم الاتصال بالبث بعد' +
            '<a href="../../games.html" class="cn-conn-link">اتصل من مكتبة الألعاب ↗</a></div>';
    }

    function refreshConnectionBadge() {
        var wrap = el('cn-conn-status-wrap');
        if (!wrap) return; // مو شاشة الإعدادات حالياً
        wrap.innerHTML = connectionBadgeHtml();
        var connectBtn = el('cn-connect-btn');
    ...
    function renderSettingsScreen() {
        ...
        root.innerHTML =
            ...
            '<div class="cn-conn-status-wrap" id="cn-conn-status-wrap">' + connectionBadgeHtml() + '</div>' +
    ...
    function playerCardInnerHtml(p, extraPillContent) {
        // لاعب يملك إطار (p.frame) يُعرض بالنظام المشترك الحقيقي
        // (showFrame:true)؛ لاعب بدون إطار يبقى ببطاقتنا المخصصة.
    ...
    // الصوت -- Audio عناصر حقيقية. ⚠️ عدم استدعاء play() إطلاقاً لو مستوى
    // الصوت صفر، لأن مجرد الاستدعاء على iOS يسكت أي صوت خلفية شغّال بجهاز
    // الاستريمر.
    ...
    // منفصلة تماماً عن بروتوكول التيك توك (اسم يدوي + طلب + موافقة يدوية).
    ...
    function generateBoard(startingTeam) {
        // كلمات اللوحة السابقة (لو موجودة) تُستبعد بالكامل من الاختيار --
        // يضمن أن الريست/المباراة الجديدة تجيب كلمات مختلفة 100%.
    ...
    // (ما يُمسَح مع كل إعادة رسم)، يظهر مؤقتاً ثم يختفي تلقائياً.
    ...
    // يرجع 'continue' لو يقدر يكمل فتح صناديق ثانية بنفس رسالة التأكيد،
    // أو 'stop' لو لازم يتوقف فورًا (محايد/فريق ثاني/قاتل).
    ...
    // مؤقّتان مسمّيان لا يشتغلان بنفس الوقت أبداً: وقت التلميح (قبل ما
    // السباي ماستر يرسل)، ووقت الاختيار/التأكيد (بعد إرسال التلميح).
