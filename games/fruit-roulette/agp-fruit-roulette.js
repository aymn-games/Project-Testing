/**
 * AGP FRUIT ROULETTE — "روليت الفواكه" (لعبة أصلية داخل المنصة، بنمط
 * games/elimination-roulette/agp-elimination-roulette.js: لا نافذة
 * خارجية ولا postMessage). واجهتها (index.html + style.css) ثابتة
 * (Static) بدل بنائها بالكامل عبر JS، بطلب صريح لصاحب المشروع.
 *
 * اختيار الصندوق: صاحب الدور (الفائز بدورة العجلة) يكتب رقم صندوقه
 * (١-٤) بشات البث، والنظام يتحقق أنه فعلاً نفس صاحب الدور (id/name)
 * قبل التنفيذ. النقر اليدوي على الصندوق يبقى شغّالاً أيضاً كمدخل موازٍ.
 * رقم كل صندوق مجرد معرّف ثابت لموضعه، لا علاقة له بمحتواه (آمن/خفي).
 *
 * الإقصاء داخل اللعبة (فتح صندوق "الشخصية الخفية") لا يساوي حذف اللاعب
 * من قائمة AGP العامة -- مصفوفة _alive داخلية خاصة باللعبة، تستمع لـ
 * player:removed فقط للحذف الإداري الحقيقي من شاشة الإعدادات.
 *
 * الاعتماديات (بنفس ترتيب index.html القياسي): js/agp-core.js …
 * js/agp-bootstrap.js، ثم js/agp-player-card.js، js/agp-entrance.js،
 * js/agp-game-shell.js، ثم هذا الملف.
 */
...
(function (AGP) {
    ...
    // قائمة هدايا الإنعاش -- نسخة مطابقة لنفس القائمة المستخدَمة بروليت
    // الإقصاء (كل لعبة ملف مستقل، لا استيراد مشترك). الهدايا اللي عندها
    // iconId تستخدم أيقونة تيك توك الحقيقية المشتركة (assets/tiktok-gifts/)
    // — تطابق مؤكد بالاسم وعدد الكوينز؛ الباقي على Twemoji (MIT + CC-BY 4.0)
    ...
    // مهلة إضافية قبل إعلان الفائز نهائياً عند آخر إقصاء -- تعطي فرصة
    // لهدية إنعاش "بالطريق" (تأخير شبكة طبيعي) تنقذ آخر لاعب مُقصى.
    // راجع checkGameOver() أدناه.
    ...
    function cacheDom() {
        ...
        spinBtn = el('spinBtn'); // مركز العجلة هو زر التدوير الوحيد؛ hub يشير لنفس العنصر
        hub = spinBtn;
    ...
    // صورة تيك توك الحقيقية للاعب (أو حرفين من اسمه كبديل).
    ...
    // مؤقت الجولة (تصاعد صعوبة الصناديق) -- المدة تُقرأ من إعدادات المباراة.
    ...
    // إشعار مرئي فعلياً بمنتصف الشاشة (بعكس liveRegion المخصَّص لقارئات
    // الشاشة فقط وغير مرئي بصرياً). يُغلَق يدوياً بزر.
