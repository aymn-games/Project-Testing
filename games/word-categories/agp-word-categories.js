/**
 * AGP GAME: اسم و حيوان و نبات و جماد و بلاد (Word Categories). لعبة
 * "Native" مستقلة (بنمط games/fruit-roulette): تحمّل AGP Core +
 * js/agp-game-shell.js، ثم هذا الملف يسجّل اللعبة ويدير كامل منطقها.
 *
 * الفكرة: حرف عربي عشوائي كل جولة، اللاعبون يكتبون إجاباتهم بالشات خلال
 * وقت محدد، ثم يراجع المضيف كل إجابة (قبول/رفض) ويُحتسب نقطة واحدة لكل
 * فئة مقبولة. الفوز إما بالوصول لنقاط مستهدفة أو بعد عدد جولات محدد.
 *
 * ⚠️ الفئات المفعَّلة عناصر تحكم حيّة داخل لوحة تحكم اللعبة نفسها (لا
 * علاقة لها بشاشة الإعدادات المشتركة)، تُطبَّق من الجولة القادمة.
 *
 * ⚠️ بروتوكول الإجابة بالشات: رسالة تبدأ بـ"-"، إجابات مفصولة بفواصل
 * (, أو ،) بنفس ترتيب الفئات المفعَّلة. فئة واحدة فقط؟ الرسالة كاملة
 * تُقرأ كإجابة واحدة. كل رسالة جديدة من نفس اللاعب تستبدل السابقة.
 */
...
(function (AGP) {
    ...
    // استبعاد الحروف الصعبة سلوك تلقائي دائم (بدون خيار بشاشة الإعدادات).
    ...
    // فئات مفعَّلة حيّة — تحكّم كامل من لوحة تحكم اللعبة نفسها.
    ...
    // محدِّد الفئات الحي — كل ضغطة تبدّل حالة فئة واحدة فوراً؛ زر "الكل"
    // يفعّل الخمس دفعة وحدة. التعديل يُطبَّق من الجولة القادمة تلقائياً.
    ...
    // اختيار حرف عشوائي — يستبعد الحروف الصعبة دائماً، ويتجنّب أي حرف
    // طلع سابقاً بنفس المباراة قدر الإمكان (يسمح بالتكرار لو نفد الخيار).
    ...
    // تفسير رسالة شات واردة إلى كائن {categoryKey: نص}. الفواصل المقبولة:
    // فاصلة (، أو ,) أو شرطة (-) أو كلمة "و" محاطة بمسافات (حتى ما تنقطع
    // كلمة تبدأ فعلياً بحرف و مثل "وردة"). بدون فاصل صريح، يُفصل بالمسافات.
    ...
    // تطبيع بسيط للمقارنة بين إجابتين (اكتشاف التكرار)، لا يلمس الإجابة
    // الأصلية المعروضة.
    ...
    // التحكم بالمراحل المرئية الثلاث. ⚠️ style.css يفرض
    // `[hidden]{display:none!important}` صراحة -- بدونها، قواعد CSS
    // الأخرى (display:flex/grid) تفرض تخصيصاً أعلى من [hidden] الافتراضية
    // وتبقي العنصر ظاهراً رغم hidden=true.
    ...
    // "تخطي الحرف" — يسحب حرف جديد فوراً بدون احتساب جولة (رقم الجولة
    // يرجع لنفس قيمته قبل السحب هنا، ثم startRound() نفسها ترجع تزيده).
    ...
    // الاستماع لتعليقات البث أثناء مرحلة الجمع فقط. يُعتمَد فقط تعليق
    // يبدأ بعلامة "-" (ملتصقة أو بمسافة) حتى ما تُقرأ دردشة عامة كإجابة.
    ...
    // إجابة مكرّرة (نفس الكلمة بالضبط بنفس الفئة، بعد تطبيع بسيط) بين أكثر
    // من لاعب: أول لاعب كتبها تحسب له نقطة كاملة لو قُبلت، وأي لاعب بعده
    ...
    // اعتماد نتائج الجولة. النقاط تُسجَّل حتى لو صفر -- ضروري حتى يظهر
    // كل من شارك بلوحة الصدارة، ويصير ممكن تحديد "آخر شخص بالترتيب".
    ...
    // نقاط المنصة الحقيقي (window.AGPAuth) لكل المشاركين.
    function finalizeMatchAndShowWinner(winnerRow) {
        ...
        // ⚠️ ثلاث حالات محتملة يقرأها openWinnerModal: null = تعذّر الاتصال
    ...
    // حقول شاشة الإعدادات المشتركة. حقول الفئات واستبعاد الحروف الصعبة
    // ليست هنا (تحكّم حي داخل اللعبة نفسها، راجع renderCategorySelector).
    ...
    // ⚠️ التحقق من وجود حقل اليوزرنيم (#agp-tiktok-username) قبل إدراج
    // الزر ضروري: موجود حصراً بشاشة الإعدادات الأولى، وليس بالنسخة
    // المُعاد فتحها منتصف المباراة (isReopened) -- بدونه الزر يظهر هناك
    // أيضاً بالخطأ لأن كلا الشاشتين تشترك بكلاس .agp-shell-btn-connect.
    function enhanceSettingsScreen() {
        ...
        if (!document.getElementById('agp-tiktok-username')) return;
