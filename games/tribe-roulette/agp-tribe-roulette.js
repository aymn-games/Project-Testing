/**
 * AGP TRIBE ROULETTE — "روليت القبائل" (لعبة أصلية داخل المنصة). لعبة
 * Native، لا نافذة خارجية ولا postMessage.
 *
 * ⚠️ مبني على أساس games/elimination-roulette/agp-elimination-roulette.js
 *   (نفس الهيكلة، شاشة الإعدادات/اللوبي/الفائز، وآلية إرجاع "وقفت العجلة
 *   على نفس الاسم مرتين"). الفروقات الجوهرية:
 *   1) عجلة بعنصر <canvas> (لا Conic Gradient بCSS).
 *   2) تبويب الاختيار: بطاقات المرشَّحين تُظهر اسم قبيلة عشوائي + شخصية
 *      مخفية، لا صورة/اسم اللاعب الحقيقي (صاحب الدور نفسه يبقى بهويته
 *      الحقيقية -- التمويه للأهداف فقط). لا فتحة "أمان"؛ كل اختيار يقصي.
 *   3) تبويب الإرجاع: هوية اللاعبين المُقصَين ظاهرة بالكامل، لكن ترتيب كل
 *      بطاقة يُعاد خلطه عشوائياً (Fisher-Yates) في كل فتحة للنافذة.
 *   4) إعلان النتيجة يكشف الهوية الحقيقية دائماً -- التمويه خاص بلحظة
 *      الاختيار فقط.
 *
...
(function (AGP) {
    ...
    // ألوان "روليت القبائل" الخاصة -- سماوي/بنفسجي مطابق لعجلة اللعبة
    // القديمة، بدل بنفسجي/سماوي روليت الإقصاء الأصلي.
    ...
    // نسخة غامقة من نفس ألوان العجلة أعلاه (كل لون بسطوع ~50%).
    ...
    function injectStageStyles() {
        ...
        style.textContent = [
            ...
            'font-family:Almarai,Cairo,sans-serif;}',
            // ⚠️ تبويب "اختيار الإقصاء" تحديداً يتميّز بحدّ وعنوان
            ...
            // ⚠️ زرّا تحكّم يدوي جديدان للمضيف — يظهران فقط بنافذة
            ...
            /* ---- بطاقات مرشَّحين أُعيد تصميمها مرة أخرى — شبكة
            ...
            // ⚠️ تكبير بطاقات المرشَّحين بطلب صريح (أرقام محددة من
            ...
            // ⚠️ رقم بطاقة "الإقصاء" تحديداً صار أخضر مميَّز (بدل
            ...
            // ⚠️ طلب صريح: رقم البادج الأخضر (الإقصاء) تحديداً
            ...
            // ⚠️ شارة "🔴 مرحلة الإقصاء" — تظهر فقط بأعلى نافذة
            ...
            'flex-shrink:0;}',
            // ⚠️ المستخدم طلب "1.20em و 17px" لخط اسم المرشَّح —
            ...
            // ⚠️ تدرّج جديد خاص باللوبي فقط (5D336A→000000 —
            ...
            // ⚠️ صف أزرار اللوبي السفلي — الثلاثة أزرار (العودة
    ...
    function renderStage() {
        ...
        // ⚠️ أسماء اللاعبين رجعت مكتوبة داخل قطع العجلة نفسها
        // (renderWheelLabels)، وزر "إعادة ترتيب عشوائية" تحت العجلة.
        // ⚠️ موشر تكبير/تصغير العجلة بين العجلة والزر — عنصر
    ...
    /**
     * ⚠️ نفس المبدأ الموروث من روليت الإقصاء (راجع الشرح
    ...
    // ⚠️ البانر صار مخفياً افتراضياً (راجع CSS tr-log-visible) —
    ...
    function buildSettingsFields() {
        return [
            ...
            {
                // ⚠️ نص توضيحي مختصر أُضيف للتسمية نفسها (لا يوجد
