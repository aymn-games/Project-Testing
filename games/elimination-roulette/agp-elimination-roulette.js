    ...
    function endMatch(winner) {
        ...
        if (window.AGPAuth && typeof window.AGPAuth.reportRoundCompletion === 'function') {
            ...
            var participants = allPlayers.map(function (p) {
                // ⚠️ [إصلاح باگ حقيقي] كان يُرسَل هنا p.name (اسم العرض
                // القابل للتغيير بتيك توك) أو p.id بصيغته الداخلية الكاملة
                // "tiktok:اليوزرنيم" — كلاهما لا يطابق أبداً tiktok_username
                // المخزَّن بحساب المنصة (يُقارَن حرفياً بالباك إند)، فالنقاط
                // كانت تفشل بصمت لكل مشارك تقريباً. الإصلاح: نشيل بادئة
                // "tiktok:" من p.id ونرسل اليوزرنيم الخام الحقيقي فقط.
                var rawUsername = (p.id || '').indexOf('tiktok:') === 0 ? p.id.slice(7) : p.id;
                return {
                    tiktokUsername: rawUsername,
