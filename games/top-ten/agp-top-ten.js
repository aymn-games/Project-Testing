/**
 * خمّن العشرة (Guess the Ten) — لعبة أصلية (Native) مستقلة بالكامل، بنفس
 * فلسفة games/team-war (لا اعتماد على js/agp-game-shell.js).
 *
 * سؤال واحد كل جولة، وعشرة صناديق مرتبة 1-10: صندوق 1 = 10 نقاط، ينزل
 * نقطة لكل صندوق حتى صندوق 10 = نقطة واحدة (ترتيب الصناديق = ترتيب أهمية
 * الإجابة، مو ترتيب اكتشافها). أي متسابق منضم يكتب إجابة تطابق صندوقاً
 * متبقياً (مطابقة تلقائية عبر تطبيع النص العربي) يفوز به فوراً.
 *
 * ⚠️ بنك الأسئلة خارجي (games/top-ten/questions-bank.json)، يُدار حصراً
 *   من admin-questions.html. لو تعذّر تحميله، تُستخدَم
 *   BUILTIN_FALLBACK_QUESTIONS أدناه حتى لا تنهار اللعبة.
 */
