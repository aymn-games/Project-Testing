/**
 * AGP STREAMER LEVEL SERVICE — نظام "SP" (نقاط الستريمر)، منفصل تماماً عن
 * points-service.js (ذاك لنقاط/مستوى اللاعب، مبني على جولات الألعاب).
 * هذا الملف لمستوى الستريمر، مبني على إحصائيات جدول broadcasts.
 *
 * ⚠️ لا توجد بيانات بث تاريخية قبل تفعيل هذا النظام — كل ستريمر يبدأ من
 * SP = صفر لحظة أول بث بعد التحديث، بصرف النظر عن نشاطه قبل ذلك. متوقَّع.
 *
 * SP = (دقائق البث الفعلية × SP_PER_LIVE_MINUTE) + (قيمة الهدايا بالماس × SP_PER_DIAMOND)
 *
 * العتبات (كم SP لكل مستوى) ليست أرقاماً بالكود — مخزَّنة بجدول
 * streamer_levels وقابلة للتعديل من لوحة الأدمن دون تعديل كود.
 */
